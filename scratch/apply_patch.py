                    ...
                    if self.active:
                        for p in pages:
                            ...
                            try:
                                ...
                                for node in nodes:
                                    ...
                                    if name and role in ["StaticText", "button", "link"]:
                                        ...
                                        if role in ["button", "link"] or (role == "StaticText" and "run" in name_lower):
                                            ...
                                            kw_match = None
                                            is_numbered = bool(re.match(r'^\d+\s+', name_lower))
                                ...
                                if actionable:
                                    # Sort so that non-submit options are clicked first
                                    actionable.sort(key=lambda t: 1 if t['kw'] == 'submit' else 0)
...
def main():
    ...
    # 1. Remove FINDER_JS lines 375-377
    content = re.sub(r"_scanner_path = os\.path\.join.*?FINDER_JS = _f\.read\(\)\n", "", content, flags=re.DOTALL)

    # 2. Replace worker_loop
    start_idx = content.find("    def worker_loop(self):")
    end_idx = content.find("    def run(self):")
    if start_idx != -1 and end_idx != -1:
        content = content[:start_idx] + AX_WORKER + "\n" + content[end_idx:]
    else:
        print("Could not find worker_loop bounds!")
